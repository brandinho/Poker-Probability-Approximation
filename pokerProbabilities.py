...
#tester_hand = np.array(['6_S', '6_H'])
#testStatus = findHandStatus(tester_hand, [])
#calcProbs(tester_hand, "Straight Flush", "PreFlop", testStatus)

def calcOpponentProbs(hand, rankType, cardsOnTable, handStatus):
    ...
    if cardsOnTable == "PreFlop":
        totalCombinations = nCr(50, 7)
        if rankType == "Pair":
            if NonPair == True:
                probYourCards = 2 * nCr(3, 2) * nCr(11, 4) * 4**4 * 3
                probDiffCards1 = 11 * nCr(4, 2) * nCr(10, 5) * 4**5
                probDiffCards2 = 11 * nCr(4, 2) * nCr(10, 3) * 4**3 * nCr(2, 2) * 3**2
                probDiffCards3 = 11 * nCr(4, 2) * nCr(10, 4) * 4**4 * nCr(2, 1) * 3
                marginalProbability1 = probYourCards / totalCombinations
                marginalProbability2 = marginalProbability1 + (1 - marginalProbability1) * (probDiffCards1 / totalCombinations)
                marginalProbability3 = marginalProbability2 + (1 - marginalProbability2) * (probDiffCards2 / totalCombinations)
                probability = marginalProbability3 + (1 - marginalProbability3) * (probDiffCards3 / totalCombinations)
            elif Pair == True:
                probYourCards = 2 * nCr(12, 5) * 4**5
                probDiffCards = 12 * nCr(4, 2) * nCr(11, 5) * 4 ** 5
                marginalProbability1 = probYourCards / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probDiffCards / totalCombinations)

        elif rankType == "Two Pair":
            if NonPair == True:
                probBothYourCards = nCr(3, 2) * nCr(11, 3) * 4**3
                probBothDiffCards = nCr(11, 2) * nCr(4, 2)**2 * nCr(9, 3) * 4**3
                probMatching_HalfYours_HalfDiff = 2 * 3 * 11 * 4 * nCr(10, 3) * 4**3
                marginalProbability1 = probBothYourCards / totalCombinations
                marginalProbability2 = marginalProbability1 + (1 - marginalProbability1) * (probBothDiffCards / totalCombinations)
                probability = marginalProbability2 + (1 - marginalProbability2) * (probMatching_HalfYours_HalfDiff / totalCombinations)
            elif Pair == True:
                probYourCards = nCr(2, 2) * 12 * nCr(4, 2) * nCr(11, 3) * 4**3
                probDiffCards = nCr(12, 2) * nCr(4, 2)**2 * nCr(10, 3) * 4**3
                marginalProbability1 = probYourCards / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probDiffCards / totalCombinations)
                
        elif rankType == "Three of a Kind":
            if NonPair == True:
                probYourCards1 = 2 * nCr(3, 3) * nCr(11, 4) * 4**4
                probYourCards2 = 2 * nCr(3, 3) * nCr(11, 3) * 4**3 * 3
                probDiffCards1 = 11 * nCr(4, 3) * nCr(10, 4) * 4**4
                probDiffCards2 = 11 * nCr(4, 3) * nCr(10, 3) * 4**4 * 3
                marginalProbability1 = probYourCards1 / totalCombinations
                marginalProbability2 = marginalProbability1 + (1 - marginalProbability1) * (probYourCards2 / totalCombinations)
                marginalProbability3 = marginalProbability2 + (1 - marginalProbability1) * (probDiffCards1 / totalCombinations)
                probability = marginalProbability3 + (1 - marginalProbability3) * (probDiffCards2 / totalCombinations)
            elif Pair == True:
                probYourCardsTable = 12 * nCr(4, 3) * nCr(11, 4) * 4**4
                probNoneYourCardsTable = 12 * nCr(4, 3) * nCr(11, 3) * 4**3 * 2
                marginalProbability1 = probYourCardsTable / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probNoneYourCardsTable / totalCombinations)

        elif rankType == "Straight":
            # Below is a super naive implementation that doesn't take into consideration what we have in our hand            
            probability = ((10 * 4**5 - 10 * 4) * nCr(8, 2) * 4**2) / nCr(52, 7)#totalCombinations
#            if straightGap == 0:
#                if straightLowerBound > 3 or straightUpperBound < 12:
#                    firstMultiplier = 4
#                    secondMultiplier = 0
...
#                probability = (firstMultiplier * 3 * 4**3 * nCr(8, 2) * 4**2) / totalCombinations             
            ...
            if numSuited == 1: 
                probFlushWithOneOfYourSuits = 2 * nCr(12, 5) * nCr(45, 2)
                probFlushWithoutYourSuits = 2 * nCr(13, 5) * nCr(45, 2)
                marginalProbability1 = probFlushWithOneOfYourSuits / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probFlushWithoutYourSuits / totalCombinations)
            elif numSuited == 2:
                probFlushWithYourSuits = nCr(11, 5) * nCr(45, 2)
                probFlushWithoutYourSuits = 3 * nCr(13, 5) * nCr(45, 2)
                marginalProbability1 = probFlushWithYourSuits / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probFlushWithoutYourSuits / totalCombinations)
        elif rankType == "Full House":
            if NonPair == True:
                probMatchingHand = 2 * nCr(3, 2) * 2 * nCr(3, 3) * nCr(11, 2) * 4**2
                probMatchingTable1 = nCr(11, 2) * nCr(4, 2) * nCr(4, 3) * nCr(9, 2) * 4**2
                probMatchingTable2 = nCr(11, 2) * nCr(4, 2) * nCr(4, 3) * 9 * 4 * 3
                probMatchingTable3 = nCr(11, 2) * nCr(4, 2) * nCr(4, 3) * 3**2
                
                probMatching_HandPair_TableTriple1 = 2 * nCr(3, 2) * 11 * nCr(4, 3) * nCr(10, 2) * 4**2
                probMatching_HandPair_TableTriple2 = 2 * nCr(3, 2) * 11 * nCr(4, 3) * 10 * 4 * 3
                probMatching_HandTriple_TablePair1 = 2 * nCr(3, 3) * 11 * nCr(4, 2) * nCr(10, 2) * 4**2
                probMatching_HandTriple_TablePair2 = 2 * nCr(3, 3) * 11 * nCr(4, 2) * 10 * 4 * 3
                
                marginalProbability1 = probMatchingHand / totalCombinations
                marginalProbability2 = marginalProbability1 + (1 - marginalProbability1) * (probMatchingTable1 / totalCombinations)
                marginalProbability3 = marginalProbability2 + (1 - marginalProbability2) * (probMatchingTable2 / totalCombinations)
                marginalProbability4 = marginalProbability3 + (1 - marginalProbability3) * (probMatchingTable3 / totalCombinations)
                marginalProbability5 = marginalProbability4 + (1 - marginalProbability4) * (probMatching_HandPair_TableTriple1 / totalCombinations)
                marginalProbability6 = marginalProbability5 + (1 - marginalProbability5) * (probMatching_HandPair_TableTriple2 / totalCombinations)
                marginalProbability7 = marginalProbability6 + (1 - marginalProbability6) * (probMatching_HandTriple_TablePair1 / totalCombinations)
                probability = marginalProbability7 + (1 - marginalProbability7) * (probMatching_HandTriple_TablePair2 / totalCombinations)                
            elif Pair == True:
                probMatching_HandPair_TableTriple = nCr(2, 2) * 12 * nCr(4, 3) * nCr(11, 2) * 4**2
                probMatching_EverythingTable = nCr(12, 2) * nCr(4, 2) * nCr(4, 3) * nCr(10, 2) * 4**2
                marginalProbability1 = probMatching_HandPair_TableTriple / totalCombinations
                probability = marginalProbability1 + (1 - marginalProbability1) * (probMatching_EverythingTable / totalCombinations) 
#        elif rankType == "Four of a Kind":
#            if NonPair == True:
#                probMatchingHand = 2 * nCr(11, 2) * 4**2
#                probMatchingTable = 11 * 10 * 4
